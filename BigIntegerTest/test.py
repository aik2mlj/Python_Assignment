...
os.makedirs("temp")
for i in range(20):
    inst ="./tryBigint BigIntegerTest"+str(i)+".in > temp/test"+str(i)+".out"
    print(inst)
    os.system(inst)
    if not filecmp.cmp("BigIntegerTest"+str(i)+".out","temp/test"+str(i)+".out"):
        f = open("BigIntegerTest" + str(i) + ".in", mode='r')
        title = f.readline()[1:]
